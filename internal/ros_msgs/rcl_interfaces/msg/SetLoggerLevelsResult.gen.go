...
type SetLoggerLevelsResult struct {
	Successful bool   `yaml:"successful"` // True when succeed, false when failed.
	Reason     string `yaml:"reason"`     // Reason why the setting was either successful or a failure.
}
...
func NewSetLoggerLevelsResultPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*SetLoggerLevelsResultPublisher, error) {
	pub, err := node.NewPublisher(topicName, SetLoggerLevelsResultTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// SetLoggerLevelsResult
func NewSetLoggerLevelsResultSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback SetLoggerLevelsResultSubscriptionCallback) (*SetLoggerLevelsResultSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg SetLoggerLevelsResult
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, SetLoggerLevelsResultTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CSetLoggerLevelsResultSequence = C.rcl_interfaces__msg__SetLoggerLevelsResult__Sequence

func SetLoggerLevelsResultSequenceToGo(goSlice *[]SetLoggerLevelsResult, cSlice CSetLoggerLevelsResultSequence) {
	if cSlice.size == 0 {
		return
	}
...
func SetLoggerLevelsResultSequenceToC(cSlice *CSetLoggerLevelsResultSequence, goSlice []SetLoggerLevelsResult) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func SetLoggerLevelsResultArrayToGo(goSlice []SetLoggerLevelsResult, cSlice []CSetLoggerLevelsResult) {
	for i := 0; i < len(cSlice); i++ {
		SetLoggerLevelsResultTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func SetLoggerLevelsResultArrayToC(cSlice []CSetLoggerLevelsResult, goSlice []SetLoggerLevelsResult) {
