...
type LoggerLevel struct {
	Name  string `yaml:"name"`  // The logger name.
	Level uint32 `yaml:"level"` // The logger level
}
...
// LoggerLevel
func NewLoggerLevelPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*LoggerLevelPublisher, error) {
	pub, err := node.NewPublisher(topicName, LoggerLevelTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// LoggerLevel
func NewLoggerLevelSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback LoggerLevelSubscriptionCallback) (*LoggerLevelSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg LoggerLevel
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, LoggerLevelTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CLoggerLevelSequence = C.rcl_interfaces__msg__LoggerLevel__Sequence

func LoggerLevelSequenceToGo(goSlice *[]LoggerLevel, cSlice CLoggerLevelSequence) {
	if cSlice.size == 0 {
		return
	}
...
func LoggerLevelSequenceToC(cSlice *CLoggerLevelSequence, goSlice []LoggerLevel) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func LoggerLevelArrayToGo(goSlice []LoggerLevel, cSlice []CLoggerLevel) {
	for i := 0; i < len(cSlice); i++ {
		LoggerLevelTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func LoggerLevelArrayToC(cSlice []CLoggerLevel, goSlice []LoggerLevel) {
