...
// ParameterValue
func NewParameterValuePublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*ParameterValuePublisher, error) {
	pub, err := node.NewPublisher(topicName, ParameterValueTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// ParameterValue
func NewParameterValueSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback ParameterValueSubscriptionCallback) (*ParameterValueSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg ParameterValue
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, ParameterValueTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
func (t _ParameterValueTypeSupport) AsCStruct(dst unsafe.Pointer, msg types.Message) {
	...
	primitives.ByteSequenceToC((*primitives.CByteSequence)(unsafe.Pointer(&mem.byte_array_value)), m.ByteArrayValue)
	primitives.BoolSequenceToC((*primitives.CBoolSequence)(unsafe.Pointer(&mem.bool_array_value)), m.BoolArrayValue)
	primitives.Int64SequenceToC((*primitives.CInt64Sequence)(unsafe.Pointer(&mem.integer_array_value)), m.IntegerArrayValue)
	primitives.Float64SequenceToC((*primitives.CFloat64Sequence)(unsafe.Pointer(&mem.double_array_value)), m.DoubleArrayValue)
	primitives.StringSequenceToC((*primitives.CStringSequence)(unsafe.Pointer(&mem.string_array_value)), m.StringArrayValue)
}
...
func (t _ParameterValueTypeSupport) AsGoStruct(msg types.Message, ros2_message_buffer unsafe.Pointer) {
	...
	primitives.ByteSequenceToGo(&m.ByteArrayValue, *(*primitives.CByteSequence)(unsafe.Pointer(&mem.byte_array_value)))
	primitives.BoolSequenceToGo(&m.BoolArrayValue, *(*primitives.CBoolSequence)(unsafe.Pointer(&mem.bool_array_value)))
	primitives.Int64SequenceToGo(&m.IntegerArrayValue, *(*primitives.CInt64Sequence)(unsafe.Pointer(&mem.integer_array_value)))
	primitives.Float64SequenceToGo(&m.DoubleArrayValue, *(*primitives.CFloat64Sequence)(unsafe.Pointer(&mem.double_array_value)))
	primitives.StringSequenceToGo(&m.StringArrayValue, *(*primitives.CStringSequence)(unsafe.Pointer(&mem.string_array_value)))
}
...
type CParameterValueSequence = C.rcl_interfaces__msg__ParameterValue__Sequence

func ParameterValueSequenceToGo(goSlice *[]ParameterValue, cSlice CParameterValueSequence) {
	if cSlice.size == 0 {
		return
	}
...
func ParameterValueSequenceToC(cSlice *CParameterValueSequence, goSlice []ParameterValue) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func ParameterValueArrayToGo(goSlice []ParameterValue, cSlice []CParameterValue) {
	for i := 0; i < len(cSlice); i++ {
		ParameterValueTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func ParameterValueArrayToC(cSlice []CParameterValue, goSlice []ParameterValue) {
