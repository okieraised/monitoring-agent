...
// Log
func NewLogPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*LogPublisher, error) {
	pub, err := node.NewPublisher(topicName, LogTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// Log
func NewLogSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback LogSubscriptionCallback) (*LogSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg Log
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, LogTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CLogSequence = C.rcl_interfaces__msg__Log__Sequence

func LogSequenceToGo(goSlice *[]Log, cSlice CLogSequence) {
	if cSlice.size == 0 {
		return
	}
...
func LogSequenceToC(cSlice *CLogSequence, goSlice []Log) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func LogArrayToGo(goSlice []Log, cSlice []CLog) {
	for i := 0; i < len(cSlice); i++ {
		LogTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func LogArrayToC(cSlice []CLog, goSlice []Log) {
