...
func NewSetParametersAtomically_RequestPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*SetParametersAtomically_RequestPublisher, error) {
	pub, err := node.NewPublisher(topicName, SetParametersAtomically_RequestTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// SetParametersAtomically_Request
func NewSetParametersAtomically_RequestSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback SetParametersAtomically_RequestSubscriptionCallback) (*SetParametersAtomically_RequestSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg SetParametersAtomically_Request
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, SetParametersAtomically_RequestTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CSetParametersAtomically_RequestSequence = C.rcl_interfaces__srv__SetParametersAtomically_Request__Sequence

func SetParametersAtomically_RequestSequenceToGo(goSlice *[]SetParametersAtomically_Request, cSlice CSetParametersAtomically_RequestSequence) {
	if cSlice.size == 0 {
		return
	}
...
func SetParametersAtomically_RequestSequenceToC(cSlice *CSetParametersAtomically_RequestSequence, goSlice []SetParametersAtomically_Request) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func SetParametersAtomically_RequestArrayToGo(goSlice []SetParametersAtomically_Request, cSlice []CSetParametersAtomically_Request) {
	for i := 0; i < len(cSlice); i++ {
		SetParametersAtomically_RequestTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func SetParametersAtomically_RequestArrayToC(cSlice []CSetParametersAtomically_Request, goSlice []SetParametersAtomically_Request) {
