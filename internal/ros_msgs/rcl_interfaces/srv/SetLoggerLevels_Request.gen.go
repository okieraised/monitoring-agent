...
type SetLoggerLevels_Request struct {
	Levels []rcl_interfaces_msg.LoggerLevel `yaml:"levels"` // A list of loggers to set.
}
...
func NewSetLoggerLevels_RequestPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*SetLoggerLevels_RequestPublisher, error) {
	pub, err := node.NewPublisher(topicName, SetLoggerLevels_RequestTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// SetLoggerLevels_Request
func NewSetLoggerLevels_RequestSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback SetLoggerLevels_RequestSubscriptionCallback) (*SetLoggerLevels_RequestSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg SetLoggerLevels_Request
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, SetLoggerLevels_RequestTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CSetLoggerLevels_RequestSequence = C.rcl_interfaces__srv__SetLoggerLevels_Request__Sequence

func SetLoggerLevels_RequestSequenceToGo(goSlice *[]SetLoggerLevels_Request, cSlice CSetLoggerLevels_RequestSequence) {
	if cSlice.size == 0 {
		return
	}
...
func SetLoggerLevels_RequestSequenceToC(cSlice *CSetLoggerLevels_RequestSequence, goSlice []SetLoggerLevels_Request) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func SetLoggerLevels_RequestArrayToGo(goSlice []SetLoggerLevels_Request, cSlice []CSetLoggerLevels_Request) {
	for i := 0; i < len(cSlice); i++ {
		SetLoggerLevels_RequestTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func SetLoggerLevels_RequestArrayToC(cSlice []CSetLoggerLevels_Request, goSlice []SetLoggerLevels_Request) {
