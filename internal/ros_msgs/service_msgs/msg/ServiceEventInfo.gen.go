...
type ServiceEventInfo struct {
	EventType      uint8                       `yaml:"event_type"`      // The type of event this message represents
	Stamp          builtin_interfaces_msg.Time `yaml:"stamp"`           // Timestamp for when the event occurred (sent or received time)
	ClientGid      [16]byte                    `yaml:"client_gid"`      // Unique identifier for the client that sent the service requestNote, this is only unique for the current session.The size here has to match the size of rmw_dds_common/msg/Gid,but unfortunately we cannot use that message directly due to acircular dependency.
	SequenceNumber int64                       `yaml:"sequence_number"` // Sequence number for the requestCombined with the client ID, this creates a unique ID for the service transaction
}
...
// ServiceEventInfo
func NewServiceEventInfoPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*ServiceEventInfoPublisher, error) {
	pub, err := node.NewPublisher(topicName, ServiceEventInfoTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// ServiceEventInfo
func NewServiceEventInfoSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback ServiceEventInfoSubscriptionCallback) (*ServiceEventInfoSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg ServiceEventInfo
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, ServiceEventInfoTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
func (t _ServiceEventInfoTypeSupport) AsCStruct(dst unsafe.Pointer, msg types.Message) {
	...
	primitives.CharArrayToC(*(*[]primitives.CChar)(unsafe.Pointer(&cSlice_client_gid)), m.ClientGid[:])
...
func (t _ServiceEventInfoTypeSupport) AsGoStruct(msg types.Message, ros2_message_buffer unsafe.Pointer) {
	...
	cSlice_client_gid := mem.client_gid[:]
	primitives.CharArrayToGo(m.ClientGid[:], *(*[]primitives.CChar)(unsafe.Pointer(&cSlice_client_gid)))
...
type CServiceEventInfoSequence = C.service_msgs__msg__ServiceEventInfo__Sequence

func ServiceEventInfoSequenceToGo(goSlice *[]ServiceEventInfo, cSlice CServiceEventInfoSequence) {
	if cSlice.size == 0 {
		return
	}
...
func ServiceEventInfoSequenceToC(cSlice *CServiceEventInfoSequence, goSlice []ServiceEventInfo) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func ServiceEventInfoArrayToGo(goSlice []ServiceEventInfo, cSlice []CServiceEventInfo) {
	for i := 0; i < len(cSlice); i++ {
		ServiceEventInfoTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func ServiceEventInfoArrayToC(cSlice []CServiceEventInfo, goSlice []ServiceEventInfo) {
