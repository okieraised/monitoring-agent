...
func init() {
	...
	typemap.RegisterMessage("std_msgs/msg/UInt64MultiArray", UInt64MultiArrayTypeSupport)
}
...
func (t *UInt64MultiArray) GetTypeSupport() types.MessageTypeSupport {
	return UInt64MultiArrayTypeSupport
}

// UInt64MultiArrayPublisher wraps rclgo.Publisher to provide type safe helper
// functions
type UInt64MultiArrayPublisher struct {
	*rclgo.Publisher
}

// NewUInt64MultiArrayPublisher creates and returns a new publisher for the
// UInt64MultiArray
func NewUInt64MultiArrayPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*UInt64MultiArrayPublisher, error) {
	pub, err := node.NewPublisher(topicName, UInt64MultiArrayTypeSupport, options)
	if err != nil {
		return nil, err
	}
	return &UInt64MultiArrayPublisher{pub}, nil
}
...
// UInt64MultiArraySubscription wraps rclgo.Subscription to provide type safe helper
// functions
type UInt64MultiArraySubscription struct {
	*rclgo.Subscription
}

// UInt64MultiArraySubscriptionCallback type is used to provide a subscription
// handler function for a UInt64MultiArraySubscription.
...
// UInt64MultiArray
func NewUInt64MultiArraySubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback UInt64MultiArraySubscriptionCallback) (*UInt64MultiArraySubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg UInt64MultiArray
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, UInt64MultiArrayTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
	return &UInt64MultiArraySubscription{sub}, nil
}
...
func (t _UInt64MultiArrayTypeSupport) AsCStruct(dst unsafe.Pointer, msg types.Message) {
	...
	primitives.Uint64SequenceToC((*primitives.CUint64Sequence)(unsafe.Pointer(&mem.data)), m.Data)
}
...
func (t _UInt64MultiArrayTypeSupport) AsGoStruct(msg types.Message, ros2_message_buffer unsafe.Pointer) {
	...
	primitives.Uint64SequenceToGo(&m.Data, *(*primitives.CUint64Sequence)(unsafe.Pointer(&mem.data)))
}
...
type CUInt64MultiArraySequence = C.std_msgs__msg__UInt64MultiArray__Sequence

func UInt64MultiArraySequenceToGo(goSlice *[]UInt64MultiArray, cSlice CUInt64MultiArraySequence) {
	if cSlice.size == 0 {
		return
	}
	*goSlice = make([]UInt64MultiArray, cSlice.size)
	src := unsafe.Slice(cSlice.data, cSlice.size)
	for i := range src {
		UInt64MultiArrayTypeSupport.AsGoStruct(&(*goSlice)[i], unsafe.Pointer(&src[i]))
	}
}

func UInt64MultiArraySequenceToC(cSlice *CUInt64MultiArraySequence, goSlice []UInt64MultiArray) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
	...
	cSlice.data = (*C.std_msgs__msg__UInt64MultiArray)(C.malloc(C.sizeof_struct_std_msgs__msg__UInt64MultiArray * C.size_t(len(goSlice))))
	cSlice.capacity = C.size_t(len(goSlice))
	cSlice.size = cSlice.capacity
	dst := unsafe.Slice(cSlice.data, cSlice.size)
	for i := range goSlice {
		UInt64MultiArrayTypeSupport.AsCStruct(unsafe.Pointer(&dst[i]), &goSlice[i])
	}
}

func UInt64MultiArrayArrayToGo(goSlice []UInt64MultiArray, cSlice []CUInt64MultiArray) {
	for i := 0; i < len(cSlice); i++ {
		UInt64MultiArrayTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func UInt64MultiArrayArrayToC(cSlice []CUInt64MultiArray, goSlice []UInt64MultiArray) {
