...
// Float64
func NewFloat64Publisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*Float64Publisher, error) {
	pub, err := node.NewPublisher(topicName, Float64TypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// Float64
func NewFloat64Subscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback Float64SubscriptionCallback) (*Float64Subscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg Float64
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, Float64TypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CFloat64Sequence = C.std_msgs__msg__Float64__Sequence
