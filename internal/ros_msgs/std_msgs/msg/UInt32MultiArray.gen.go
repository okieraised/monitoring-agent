...
// UInt32MultiArray
func NewUInt32MultiArrayPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*UInt32MultiArrayPublisher, error) {
	pub, err := node.NewPublisher(topicName, UInt32MultiArrayTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// UInt32MultiArray
func NewUInt32MultiArraySubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback UInt32MultiArraySubscriptionCallback) (*UInt32MultiArraySubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg UInt32MultiArray
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, UInt32MultiArrayTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
func (t _UInt32MultiArrayTypeSupport) AsCStruct(dst unsafe.Pointer, msg types.Message) {
	...
	primitives.Uint32SequenceToC((*primitives.CUint32Sequence)(unsafe.Pointer(&mem.data)), m.Data)
}
...
func (t _UInt32MultiArrayTypeSupport) AsGoStruct(msg types.Message, ros2_message_buffer unsafe.Pointer) {
	...
	primitives.Uint32SequenceToGo(&m.Data, *(*primitives.CUint32Sequence)(unsafe.Pointer(&mem.data)))
}
...
type CUInt32MultiArraySequence = C.std_msgs__msg__UInt32MultiArray__Sequence

func UInt32MultiArraySequenceToGo(goSlice *[]UInt32MultiArray, cSlice CUInt32MultiArraySequence) {
	if cSlice.size == 0 {
		return
	}
...
func UInt32MultiArraySequenceToC(cSlice *CUInt32MultiArraySequence, goSlice []UInt32MultiArray) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func UInt32MultiArrayArrayToGo(goSlice []UInt32MultiArray, cSlice []CUInt32MultiArray) {
	for i := 0; i < len(cSlice); i++ {
		UInt32MultiArrayTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func UInt32MultiArrayArrayToC(cSlice []CUInt32MultiArray, goSlice []UInt32MultiArray) {
