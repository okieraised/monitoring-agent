...
// UInt8MultiArray
func NewUInt8MultiArrayPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*UInt8MultiArrayPublisher, error) {
	pub, err := node.NewPublisher(topicName, UInt8MultiArrayTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// UInt8MultiArray
func NewUInt8MultiArraySubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback UInt8MultiArraySubscriptionCallback) (*UInt8MultiArraySubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg UInt8MultiArray
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, UInt8MultiArrayTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CUInt8MultiArraySequence = C.std_msgs__msg__UInt8MultiArray__Sequence

func UInt8MultiArraySequenceToGo(goSlice *[]UInt8MultiArray, cSlice CUInt8MultiArraySequence) {
	if cSlice.size == 0 {
		return
	}
...
func UInt8MultiArraySequenceToC(cSlice *CUInt8MultiArraySequence, goSlice []UInt8MultiArray) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func UInt8MultiArrayArrayToGo(goSlice []UInt8MultiArray, cSlice []CUInt8MultiArray) {
	for i := 0; i < len(cSlice); i++ {
		UInt8MultiArrayTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func UInt8MultiArrayArrayToC(cSlice []CUInt8MultiArray, goSlice []UInt8MultiArray) {
