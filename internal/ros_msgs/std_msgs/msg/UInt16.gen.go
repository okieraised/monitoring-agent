...
// UInt16
func NewUInt16Publisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*UInt16Publisher, error) {
	pub, err := node.NewPublisher(topicName, UInt16TypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// UInt16
func NewUInt16Subscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback UInt16SubscriptionCallback) (*UInt16Subscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg UInt16
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, UInt16TypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CUInt16Sequence = C.std_msgs__msg__UInt16__Sequence
