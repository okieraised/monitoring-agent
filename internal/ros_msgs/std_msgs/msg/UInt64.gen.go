...
// UInt64
func NewUInt64Publisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*UInt64Publisher, error) {
	pub, err := node.NewPublisher(topicName, UInt64TypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// UInt64
func NewUInt64Subscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback UInt64SubscriptionCallback) (*UInt64Subscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg UInt64
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, UInt64TypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CUInt64Sequence = C.std_msgs__msg__UInt64__Sequence
