...
// UInt8
func NewUInt8Publisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*UInt8Publisher, error) {
	pub, err := node.NewPublisher(topicName, UInt8TypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// UInt8
func NewUInt8Subscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback UInt8SubscriptionCallback) (*UInt8Subscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg UInt8
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, UInt8TypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CUInt8Sequence = C.std_msgs__msg__UInt8__Sequence
