...
// UInt32
func NewUInt32Publisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*UInt32Publisher, error) {
	pub, err := node.NewPublisher(topicName, UInt32TypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// UInt32
func NewUInt32Subscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback UInt32SubscriptionCallback) (*UInt32Subscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg UInt32
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, UInt32TypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CUInt32Sequence = C.std_msgs__msg__UInt32__Sequence
