...
// UInt16MultiArray
func NewUInt16MultiArrayPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*UInt16MultiArrayPublisher, error) {
	pub, err := node.NewPublisher(topicName, UInt16MultiArrayTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// UInt16MultiArray
func NewUInt16MultiArraySubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback UInt16MultiArraySubscriptionCallback) (*UInt16MultiArraySubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg UInt16MultiArray
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, UInt16MultiArrayTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
func (t _UInt16MultiArrayTypeSupport) AsCStruct(dst unsafe.Pointer, msg types.Message) {
	...
	primitives.Uint16SequenceToC((*primitives.CUint16Sequence)(unsafe.Pointer(&mem.data)), m.Data)
}
...
func (t _UInt16MultiArrayTypeSupport) AsGoStruct(msg types.Message, ros2_message_buffer unsafe.Pointer) {
	...
	primitives.Uint16SequenceToGo(&m.Data, *(*primitives.CUint16Sequence)(unsafe.Pointer(&mem.data)))
}
...
type CUInt16MultiArraySequence = C.std_msgs__msg__UInt16MultiArray__Sequence

func UInt16MultiArraySequenceToGo(goSlice *[]UInt16MultiArray, cSlice CUInt16MultiArraySequence) {
	if cSlice.size == 0 {
		return
	}
...
func UInt16MultiArraySequenceToC(cSlice *CUInt16MultiArraySequence, goSlice []UInt16MultiArray) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func UInt16MultiArrayArrayToGo(goSlice []UInt16MultiArray, cSlice []CUInt16MultiArray) {
	for i := 0; i < len(cSlice); i++ {
		UInt16MultiArrayTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func UInt16MultiArrayArrayToC(cSlice []CUInt16MultiArray, goSlice []UInt16MultiArray) {
