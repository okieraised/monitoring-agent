...
// DiagnosticArray
func NewDiagnosticArrayPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*DiagnosticArrayPublisher, error) {
	pub, err := node.NewPublisher(topicName, DiagnosticArrayTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// DiagnosticArray
func NewDiagnosticArraySubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback DiagnosticArraySubscriptionCallback) (*DiagnosticArraySubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg DiagnosticArray
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, DiagnosticArrayTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
func (t _DiagnosticArrayTypeSupport) AsCStruct(dst unsafe.Pointer, msg types.Message) {
	...
	DiagnosticStatusSequenceToC(&mem.status, m.Status)
}
...
func (t _DiagnosticArrayTypeSupport) AsGoStruct(msg types.Message, ros2_message_buffer unsafe.Pointer) {
	...
	DiagnosticStatusSequenceToGo(&m.Status, mem.status)
}
...
type CDiagnosticArraySequence = C.diagnostic_msgs__msg__DiagnosticArray__Sequence

func DiagnosticArraySequenceToGo(goSlice *[]DiagnosticArray, cSlice CDiagnosticArraySequence) {
	if cSlice.size == 0 {
		return
	}
...
func DiagnosticArraySequenceToC(cSlice *CDiagnosticArraySequence, goSlice []DiagnosticArray) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func DiagnosticArrayArrayToGo(goSlice []DiagnosticArray, cSlice []CDiagnosticArray) {
	for i := 0; i < len(cSlice); i++ {
		DiagnosticArrayTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func DiagnosticArrayArrayToC(cSlice []CDiagnosticArray, goSlice []DiagnosticArray) {
