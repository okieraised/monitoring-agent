...
// Empty_Request
func NewEmpty_RequestPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*Empty_RequestPublisher, error) {
	pub, err := node.NewPublisher(topicName, Empty_RequestTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// Empty_Request
func NewEmpty_RequestSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback Empty_RequestSubscriptionCallback) (*Empty_RequestSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg Empty_Request
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, Empty_RequestTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CEmpty_RequestSequence = C.std_srvs__srv__Empty_Request__Sequence

func Empty_RequestSequenceToGo(goSlice *[]Empty_Request, cSlice CEmpty_RequestSequence) {
	if cSlice.size == 0 {
		return
	}
...
func Empty_RequestSequenceToC(cSlice *CEmpty_RequestSequence, goSlice []Empty_Request) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func Empty_RequestArrayToGo(goSlice []Empty_Request, cSlice []CEmpty_Request) {
	for i := 0; i < len(cSlice); i++ {
		Empty_RequestTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func Empty_RequestArrayToC(cSlice []CEmpty_Request, goSlice []Empty_Request) {
