...
// OccupancyGrid
func NewOccupancyGridPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*OccupancyGridPublisher, error) {
	pub, err := node.NewPublisher(topicName, OccupancyGridTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// OccupancyGrid
func NewOccupancyGridSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback OccupancyGridSubscriptionCallback) (*OccupancyGridSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg OccupancyGrid
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, OccupancyGridTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
func (t _OccupancyGridTypeSupport) AsCStruct(dst unsafe.Pointer, msg types.Message) {
	...
	primitives.Int8SequenceToC((*primitives.CInt8Sequence)(unsafe.Pointer(&mem.data)), m.Data)
}
...
func (t _OccupancyGridTypeSupport) AsGoStruct(msg types.Message, ros2_message_buffer unsafe.Pointer) {
	...
	primitives.Int8SequenceToGo(&m.Data, *(*primitives.CInt8Sequence)(unsafe.Pointer(&mem.data)))
}
...
type COccupancyGridSequence = C.nav_msgs__msg__OccupancyGrid__Sequence

func OccupancyGridSequenceToGo(goSlice *[]OccupancyGrid, cSlice COccupancyGridSequence) {
	if cSlice.size == 0 {
		return
	}
...
func OccupancyGridSequenceToC(cSlice *COccupancyGridSequence, goSlice []OccupancyGrid) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func OccupancyGridArrayToGo(goSlice []OccupancyGrid, cSlice []COccupancyGrid) {
	for i := 0; i < len(cSlice); i++ {
		OccupancyGridTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func OccupancyGridArrayToC(cSlice []COccupancyGrid, goSlice []OccupancyGrid) {
