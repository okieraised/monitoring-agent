...
// Path
func NewPathPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*PathPublisher, error) {
	pub, err := node.NewPublisher(topicName, PathTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// Path
func NewPathSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback PathSubscriptionCallback) (*PathSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg Path
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, PathTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
func (t _PathTypeSupport) AsCStruct(dst unsafe.Pointer, msg types.Message) {
	...
	geometry_msgs_msg.PoseStampedSequenceToC((*geometry_msgs_msg.CPoseStampedSequence)(unsafe.Pointer(&mem.poses)), m.Poses)
}
...
func (t _PathTypeSupport) AsGoStruct(msg types.Message, ros2_message_buffer unsafe.Pointer) {
	...
	geometry_msgs_msg.PoseStampedSequenceToGo(&m.Poses, *(*geometry_msgs_msg.CPoseStampedSequence)(unsafe.Pointer(&mem.poses)))
}
...
type CPathSequence = C.nav_msgs__msg__Path__Sequence

func PathSequenceToGo(goSlice *[]Path, cSlice CPathSequence) {
	if cSlice.size == 0 {
		return
	}
...
func PathSequenceToC(cSlice *CPathSequence, goSlice []Path) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func PathArrayToGo(goSlice []Path, cSlice []CPath) {
	for i := 0; i < len(cSlice); i++ {
		PathTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func PathArrayToC(cSlice []CPath, goSlice []Path) {
