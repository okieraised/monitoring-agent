...
// MapMetaData
func NewMapMetaDataPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*MapMetaDataPublisher, error) {
	pub, err := node.NewPublisher(topicName, MapMetaDataTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// MapMetaData
func NewMapMetaDataSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback MapMetaDataSubscriptionCallback) (*MapMetaDataSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg MapMetaData
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, MapMetaDataTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CMapMetaDataSequence = C.nav_msgs__msg__MapMetaData__Sequence

func MapMetaDataSequenceToGo(goSlice *[]MapMetaData, cSlice CMapMetaDataSequence) {
	if cSlice.size == 0 {
		return
	}
...
func MapMetaDataSequenceToC(cSlice *CMapMetaDataSequence, goSlice []MapMetaData) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func MapMetaDataArrayToGo(goSlice []MapMetaData, cSlice []CMapMetaData) {
	for i := 0; i < len(cSlice); i++ {
		MapMetaDataTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func MapMetaDataArrayToC(cSlice []CMapMetaData, goSlice []MapMetaData) {
