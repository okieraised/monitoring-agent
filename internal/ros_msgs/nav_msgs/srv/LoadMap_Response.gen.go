...
// LoadMap_Response
func NewLoadMap_ResponsePublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*LoadMap_ResponsePublisher, error) {
	pub, err := node.NewPublisher(topicName, LoadMap_ResponseTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// LoadMap_Response
func NewLoadMap_ResponseSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback LoadMap_ResponseSubscriptionCallback) (*LoadMap_ResponseSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg LoadMap_Response
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, LoadMap_ResponseTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CLoadMap_ResponseSequence = C.nav_msgs__srv__LoadMap_Response__Sequence

func LoadMap_ResponseSequenceToGo(goSlice *[]LoadMap_Response, cSlice CLoadMap_ResponseSequence) {
	if cSlice.size == 0 {
		return
	}
...
func LoadMap_ResponseSequenceToC(cSlice *CLoadMap_ResponseSequence, goSlice []LoadMap_Response) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func LoadMap_ResponseArrayToGo(goSlice []LoadMap_Response, cSlice []CLoadMap_Response) {
	for i := 0; i < len(cSlice); i++ {
		LoadMap_ResponseTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func LoadMap_ResponseArrayToC(cSlice []CLoadMap_Response, goSlice []LoadMap_Response) {
