...
// GetPlan_Request
func NewGetPlan_RequestPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*GetPlan_RequestPublisher, error) {
	pub, err := node.NewPublisher(topicName, GetPlan_RequestTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// GetPlan_Request
func NewGetPlan_RequestSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback GetPlan_RequestSubscriptionCallback) (*GetPlan_RequestSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg GetPlan_Request
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, GetPlan_RequestTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CGetPlan_RequestSequence = C.nav_msgs__srv__GetPlan_Request__Sequence

func GetPlan_RequestSequenceToGo(goSlice *[]GetPlan_Request, cSlice CGetPlan_RequestSequence) {
	if cSlice.size == 0 {
		return
	}
...
func GetPlan_RequestSequenceToC(cSlice *CGetPlan_RequestSequence, goSlice []GetPlan_Request) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func GetPlan_RequestArrayToGo(goSlice []GetPlan_Request, cSlice []CGetPlan_Request) {
	for i := 0; i < len(cSlice); i++ {
		GetPlan_RequestTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func GetPlan_RequestArrayToC(cSlice []CGetPlan_Request, goSlice []GetPlan_Request) {
