...
// TransitionEvent
func NewTransitionEventPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*TransitionEventPublisher, error) {
	pub, err := node.NewPublisher(topicName, TransitionEventTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// TransitionEvent
func NewTransitionEventSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback TransitionEventSubscriptionCallback) (*TransitionEventSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg TransitionEvent
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, TransitionEventTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CTransitionEventSequence = C.lifecycle_msgs__msg__TransitionEvent__Sequence

func TransitionEventSequenceToGo(goSlice *[]TransitionEvent, cSlice CTransitionEventSequence) {
	if cSlice.size == 0 {
		return
	}
...
func TransitionEventSequenceToC(cSlice *CTransitionEventSequence, goSlice []TransitionEvent) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func TransitionEventArrayToGo(goSlice []TransitionEvent, cSlice []CTransitionEvent) {
	for i := 0; i < len(cSlice); i++ {
		TransitionEventTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func TransitionEventArrayToC(cSlice []CTransitionEvent, goSlice []TransitionEvent) {
