...
// Transition
func NewTransitionPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*TransitionPublisher, error) {
	pub, err := node.NewPublisher(topicName, TransitionTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// Transition
func NewTransitionSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback TransitionSubscriptionCallback) (*TransitionSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg Transition
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, TransitionTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CTransitionSequence = C.lifecycle_msgs__msg__Transition__Sequence

func TransitionSequenceToGo(goSlice *[]Transition, cSlice CTransitionSequence) {
	if cSlice.size == 0 {
		return
	}
...
func TransitionSequenceToC(cSlice *CTransitionSequence, goSlice []Transition) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func TransitionArrayToGo(goSlice []Transition, cSlice []CTransition) {
	for i := 0; i < len(cSlice); i++ {
		TransitionTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func TransitionArrayToC(cSlice []CTransition, goSlice []Transition) {
