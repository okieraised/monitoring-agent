...
// State
func NewStatePublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*StatePublisher, error) {
	pub, err := node.NewPublisher(topicName, StateTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// State
func NewStateSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback StateSubscriptionCallback) (*StateSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg State
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, StateTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CStateSequence = C.lifecycle_msgs__msg__State__Sequence

func StateSequenceToGo(goSlice *[]State, cSlice CStateSequence) {
	if cSlice.size == 0 {
		return
	}
...
func StateSequenceToC(cSlice *CStateSequence, goSlice []State) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func StateArrayToGo(goSlice []State, cSlice []CState) {
	for i := 0; i < len(cSlice); i++ {
		StateTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func StateArrayToC(cSlice []CState, goSlice []State) {
