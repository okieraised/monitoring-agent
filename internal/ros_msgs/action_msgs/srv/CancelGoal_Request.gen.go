...
// CancelGoal_Request
func NewCancelGoal_RequestPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*CancelGoal_RequestPublisher, error) {
	pub, err := node.NewPublisher(topicName, CancelGoal_RequestTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// CancelGoal_Request
func NewCancelGoal_RequestSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback CancelGoal_RequestSubscriptionCallback) (*CancelGoal_RequestSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg CancelGoal_Request
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, CancelGoal_RequestTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CCancelGoal_RequestSequence = C.action_msgs__srv__CancelGoal_Request__Sequence

func CancelGoal_RequestSequenceToGo(goSlice *[]CancelGoal_Request, cSlice CCancelGoal_RequestSequence) {
	if cSlice.size == 0 {
		return
	}
...
func CancelGoal_RequestSequenceToC(cSlice *CCancelGoal_RequestSequence, goSlice []CancelGoal_Request) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func CancelGoal_RequestArrayToGo(goSlice []CancelGoal_Request, cSlice []CCancelGoal_Request) {
	for i := 0; i < len(cSlice); i++ {
		CancelGoal_RequestTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func CancelGoal_RequestArrayToC(cSlice []CCancelGoal_Request, goSlice []CancelGoal_Request) {
