...
type ReadSplitEvent struct {
	...
	NodeName   string `yaml:"node_name"`   // The fully qualified node name of the event sender
}
...
// ReadSplitEvent
func NewReadSplitEventPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*ReadSplitEventPublisher, error) {
	pub, err := node.NewPublisher(topicName, ReadSplitEventTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// ReadSplitEvent
func NewReadSplitEventSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback ReadSplitEventSubscriptionCallback) (*ReadSplitEventSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg ReadSplitEvent
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, ReadSplitEventTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CReadSplitEventSequence = C.rosbag2_interfaces__msg__ReadSplitEvent__Sequence

func ReadSplitEventSequenceToGo(goSlice *[]ReadSplitEvent, cSlice CReadSplitEventSequence) {
	if cSlice.size == 0 {
		return
	}
...
func ReadSplitEventSequenceToC(cSlice *CReadSplitEventSequence, goSlice []ReadSplitEvent) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func ReadSplitEventArrayToGo(goSlice []ReadSplitEvent, cSlice []CReadSplitEvent) {
	for i := 0; i < len(cSlice); i++ {
		ReadSplitEventTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func ReadSplitEventArrayToC(cSlice []CReadSplitEvent, goSlice []ReadSplitEvent) {
