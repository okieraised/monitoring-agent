...
// PlayNext_Request
func NewPlayNext_RequestPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*PlayNext_RequestPublisher, error) {
	pub, err := node.NewPublisher(topicName, PlayNext_RequestTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// PlayNext_Request
func NewPlayNext_RequestSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback PlayNext_RequestSubscriptionCallback) (*PlayNext_RequestSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg PlayNext_Request
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, PlayNext_RequestTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CPlayNext_RequestSequence = C.rosbag2_interfaces__srv__PlayNext_Request__Sequence

func PlayNext_RequestSequenceToGo(goSlice *[]PlayNext_Request, cSlice CPlayNext_RequestSequence) {
	if cSlice.size == 0 {
		return
	}
...
func PlayNext_RequestSequenceToC(cSlice *CPlayNext_RequestSequence, goSlice []PlayNext_Request) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func PlayNext_RequestArrayToGo(goSlice []PlayNext_Request, cSlice []CPlayNext_Request) {
	for i := 0; i < len(cSlice); i++ {
		PlayNext_RequestTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func PlayNext_RequestArrayToC(cSlice []CPlayNext_Request, goSlice []PlayNext_Request) {
