...
func NewTogglePaused_ResponsePublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*TogglePaused_ResponsePublisher, error) {
	pub, err := node.NewPublisher(topicName, TogglePaused_ResponseTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// TogglePaused_Response
func NewTogglePaused_ResponseSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback TogglePaused_ResponseSubscriptionCallback) (*TogglePaused_ResponseSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg TogglePaused_Response
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, TogglePaused_ResponseTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CTogglePaused_ResponseSequence = C.rosbag2_interfaces__srv__TogglePaused_Response__Sequence

func TogglePaused_ResponseSequenceToGo(goSlice *[]TogglePaused_Response, cSlice CTogglePaused_ResponseSequence) {
	if cSlice.size == 0 {
		return
	}
...
func TogglePaused_ResponseSequenceToC(cSlice *CTogglePaused_ResponseSequence, goSlice []TogglePaused_Response) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func TogglePaused_ResponseArrayToGo(goSlice []TogglePaused_Response, cSlice []CTogglePaused_Response) {
	for i := 0; i < len(cSlice); i++ {
		TogglePaused_ResponseTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func TogglePaused_ResponseArrayToC(cSlice []CTogglePaused_Response, goSlice []TogglePaused_Response) {
