...
// Pause_Request
func NewPause_RequestPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*Pause_RequestPublisher, error) {
	pub, err := node.NewPublisher(topicName, Pause_RequestTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// Pause_Request
func NewPause_RequestSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback Pause_RequestSubscriptionCallback) (*Pause_RequestSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg Pause_Request
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, Pause_RequestTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CPause_RequestSequence = C.rosbag2_interfaces__srv__Pause_Request__Sequence

func Pause_RequestSequenceToGo(goSlice *[]Pause_Request, cSlice CPause_RequestSequence) {
	if cSlice.size == 0 {
		return
	}
...
func Pause_RequestSequenceToC(cSlice *CPause_RequestSequence, goSlice []Pause_Request) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func Pause_RequestArrayToGo(goSlice []Pause_Request, cSlice []CPause_Request) {
	for i := 0; i < len(cSlice); i++ {
		Pause_RequestTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func Pause_RequestArrayToC(cSlice []CPause_Request, goSlice []Pause_Request) {
