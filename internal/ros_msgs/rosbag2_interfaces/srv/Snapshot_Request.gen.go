...
// Snapshot_Request
func NewSnapshot_RequestPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*Snapshot_RequestPublisher, error) {
	pub, err := node.NewPublisher(topicName, Snapshot_RequestTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// Snapshot_Request
func NewSnapshot_RequestSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback Snapshot_RequestSubscriptionCallback) (*Snapshot_RequestSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg Snapshot_Request
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, Snapshot_RequestTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CSnapshot_RequestSequence = C.rosbag2_interfaces__srv__Snapshot_Request__Sequence

func Snapshot_RequestSequenceToGo(goSlice *[]Snapshot_Request, cSlice CSnapshot_RequestSequence) {
	if cSlice.size == 0 {
		return
	}
...
func Snapshot_RequestSequenceToC(cSlice *CSnapshot_RequestSequence, goSlice []Snapshot_Request) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func Snapshot_RequestArrayToGo(goSlice []Snapshot_Request, cSlice []CSnapshot_Request) {
	for i := 0; i < len(cSlice); i++ {
		Snapshot_RequestTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func Snapshot_RequestArrayToC(cSlice []CSnapshot_Request, goSlice []Snapshot_Request) {
