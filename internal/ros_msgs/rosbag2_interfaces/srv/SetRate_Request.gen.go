...
// SetRate_Request
func NewSetRate_RequestPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*SetRate_RequestPublisher, error) {
	pub, err := node.NewPublisher(topicName, SetRate_RequestTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// SetRate_Request
func NewSetRate_RequestSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback SetRate_RequestSubscriptionCallback) (*SetRate_RequestSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg SetRate_Request
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, SetRate_RequestTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CSetRate_RequestSequence = C.rosbag2_interfaces__srv__SetRate_Request__Sequence

func SetRate_RequestSequenceToGo(goSlice *[]SetRate_Request, cSlice CSetRate_RequestSequence) {
	if cSlice.size == 0 {
		return
	}
...
func SetRate_RequestSequenceToC(cSlice *CSetRate_RequestSequence, goSlice []SetRate_Request) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func SetRate_RequestArrayToGo(goSlice []SetRate_Request, cSlice []CSetRate_Request) {
	for i := 0; i < len(cSlice); i++ {
		SetRate_RequestTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func SetRate_RequestArrayToC(cSlice []CSetRate_Request, goSlice []SetRate_Request) {
