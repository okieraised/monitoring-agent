...
// RegionOfInterest
func NewRegionOfInterestPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*RegionOfInterestPublisher, error) {
	pub, err := node.NewPublisher(topicName, RegionOfInterestTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// RegionOfInterest
func NewRegionOfInterestSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback RegionOfInterestSubscriptionCallback) (*RegionOfInterestSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg RegionOfInterest
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, RegionOfInterestTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CRegionOfInterestSequence = C.sensor_msgs__msg__RegionOfInterest__Sequence

func RegionOfInterestSequenceToGo(goSlice *[]RegionOfInterest, cSlice CRegionOfInterestSequence) {
	if cSlice.size == 0 {
		return
	}
...
func RegionOfInterestSequenceToC(cSlice *CRegionOfInterestSequence, goSlice []RegionOfInterest) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func RegionOfInterestArrayToGo(goSlice []RegionOfInterest, cSlice []CRegionOfInterest) {
	for i := 0; i < len(cSlice); i++ {
		RegionOfInterestTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func RegionOfInterestArrayToC(cSlice []CRegionOfInterest, goSlice []RegionOfInterest) {
