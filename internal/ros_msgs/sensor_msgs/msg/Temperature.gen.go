...
// Temperature
func NewTemperaturePublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*TemperaturePublisher, error) {
	pub, err := node.NewPublisher(topicName, TemperatureTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// Temperature
func NewTemperatureSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback TemperatureSubscriptionCallback) (*TemperatureSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg Temperature
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, TemperatureTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CTemperatureSequence = C.sensor_msgs__msg__Temperature__Sequence

func TemperatureSequenceToGo(goSlice *[]Temperature, cSlice CTemperatureSequence) {
	if cSlice.size == 0 {
		return
	}
...
func TemperatureSequenceToC(cSlice *CTemperatureSequence, goSlice []Temperature) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func TemperatureArrayToGo(goSlice []Temperature, cSlice []CTemperature) {
	for i := 0; i < len(cSlice); i++ {
		TemperatureTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func TemperatureArrayToC(cSlice []CTemperature, goSlice []Temperature) {
