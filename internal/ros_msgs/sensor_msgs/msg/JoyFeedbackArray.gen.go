...
// JoyFeedbackArray
func NewJoyFeedbackArrayPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*JoyFeedbackArrayPublisher, error) {
	pub, err := node.NewPublisher(topicName, JoyFeedbackArrayTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// JoyFeedbackArray
func NewJoyFeedbackArraySubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback JoyFeedbackArraySubscriptionCallback) (*JoyFeedbackArraySubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg JoyFeedbackArray
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, JoyFeedbackArrayTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
func (t _JoyFeedbackArrayTypeSupport) AsCStruct(dst unsafe.Pointer, msg types.Message) {
	...
	JoyFeedbackSequenceToC(&mem.array, m.Array)
}
...
func (t _JoyFeedbackArrayTypeSupport) AsGoStruct(msg types.Message, ros2_message_buffer unsafe.Pointer) {
	...
	JoyFeedbackSequenceToGo(&m.Array, mem.array)
}
...
type CJoyFeedbackArraySequence = C.sensor_msgs__msg__JoyFeedbackArray__Sequence

func JoyFeedbackArraySequenceToGo(goSlice *[]JoyFeedbackArray, cSlice CJoyFeedbackArraySequence) {
	if cSlice.size == 0 {
		return
	}
...
func JoyFeedbackArraySequenceToC(cSlice *CJoyFeedbackArraySequence, goSlice []JoyFeedbackArray) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func JoyFeedbackArrayArrayToGo(goSlice []JoyFeedbackArray, cSlice []CJoyFeedbackArray) {
	for i := 0; i < len(cSlice); i++ {
		JoyFeedbackArrayTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func JoyFeedbackArrayArrayToC(cSlice []CJoyFeedbackArray, goSlice []JoyFeedbackArray) {
