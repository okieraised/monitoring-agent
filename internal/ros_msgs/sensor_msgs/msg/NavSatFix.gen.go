...
// NavSatFix
func NewNavSatFixPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*NavSatFixPublisher, error) {
	pub, err := node.NewPublisher(topicName, NavSatFixTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// NavSatFix
func NewNavSatFixSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback NavSatFixSubscriptionCallback) (*NavSatFixSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg NavSatFix
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, NavSatFixTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
func (t _NavSatFixTypeSupport) AsCStruct(dst unsafe.Pointer, msg types.Message) {
	...
	primitives.Float64ArrayToC(*(*[]primitives.CFloat64)(unsafe.Pointer(&cSlice_position_covariance)), m.PositionCovariance[:])
...
func (t _NavSatFixTypeSupport) AsGoStruct(msg types.Message, ros2_message_buffer unsafe.Pointer) {
	...
	cSlice_position_covariance := mem.position_covariance[:]
	primitives.Float64ArrayToGo(m.PositionCovariance[:], *(*[]primitives.CFloat64)(unsafe.Pointer(&cSlice_position_covariance)))
...
type CNavSatFixSequence = C.sensor_msgs__msg__NavSatFix__Sequence

func NavSatFixSequenceToGo(goSlice *[]NavSatFix, cSlice CNavSatFixSequence) {
	if cSlice.size == 0 {
		return
	}
...
func NavSatFixSequenceToC(cSlice *CNavSatFixSequence, goSlice []NavSatFix) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func NavSatFixArrayToGo(goSlice []NavSatFix, cSlice []CNavSatFix) {
	for i := 0; i < len(cSlice); i++ {
		NavSatFixTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func NavSatFixArrayToC(cSlice []CNavSatFix, goSlice []NavSatFix) {
