...
// PointCloud2
func NewPointCloud2Publisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*PointCloud2Publisher, error) {
	pub, err := node.NewPublisher(topicName, PointCloud2TypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// PointCloud2
func NewPointCloud2Subscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback PointCloud2SubscriptionCallback) (*PointCloud2Subscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg PointCloud2
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, PointCloud2TypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
func (t _PointCloud2TypeSupport) AsCStruct(dst unsafe.Pointer, msg types.Message) {
	...
	PointFieldSequenceToC(&mem.fields, m.Fields)
	mem.is_bigendian = C.bool(m.IsBigendian)
	mem.point_step = C.uint32_t(m.PointStep)
	mem.row_step = C.uint32_t(m.RowStep)
	primitives.Uint8SequenceToC((*primitives.CUint8Sequence)(unsafe.Pointer(&mem.data)), m.Data)
...
func (t _PointCloud2TypeSupport) AsGoStruct(msg types.Message, ros2_message_buffer unsafe.Pointer) {
	...
	PointFieldSequenceToGo(&m.Fields, mem.fields)
	m.IsBigendian = bool(mem.is_bigendian)
	m.PointStep = uint32(mem.point_step)
	m.RowStep = uint32(mem.row_step)
	primitives.Uint8SequenceToGo(&m.Data, *(*primitives.CUint8Sequence)(unsafe.Pointer(&mem.data)))
...
type CPointCloud2Sequence = C.sensor_msgs__msg__PointCloud2__Sequence

func PointCloud2SequenceToGo(goSlice *[]PointCloud2, cSlice CPointCloud2Sequence) {
	if cSlice.size == 0 {
		return
	}
...
func PointCloud2SequenceToC(cSlice *CPointCloud2Sequence, goSlice []PointCloud2) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func PointCloud2ArrayToGo(goSlice []PointCloud2, cSlice []CPointCloud2) {
	for i := 0; i < len(cSlice); i++ {
		PointCloud2TypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func PointCloud2ArrayToC(cSlice []CPointCloud2, goSlice []PointCloud2) {
