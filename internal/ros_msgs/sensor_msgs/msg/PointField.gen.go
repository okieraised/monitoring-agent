...
// PointField
func NewPointFieldPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*PointFieldPublisher, error) {
	pub, err := node.NewPublisher(topicName, PointFieldTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// PointField
func NewPointFieldSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback PointFieldSubscriptionCallback) (*PointFieldSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg PointField
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, PointFieldTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CPointFieldSequence = C.sensor_msgs__msg__PointField__Sequence

func PointFieldSequenceToGo(goSlice *[]PointField, cSlice CPointFieldSequence) {
	if cSlice.size == 0 {
		return
	}
...
func PointFieldSequenceToC(cSlice *CPointFieldSequence, goSlice []PointField) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func PointFieldArrayToGo(goSlice []PointField, cSlice []CPointField) {
	for i := 0; i < len(cSlice); i++ {
		PointFieldTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func PointFieldArrayToC(cSlice []CPointField, goSlice []PointField) {
