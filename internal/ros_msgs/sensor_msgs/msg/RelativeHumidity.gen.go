...
// RelativeHumidity
func NewRelativeHumidityPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*RelativeHumidityPublisher, error) {
	pub, err := node.NewPublisher(topicName, RelativeHumidityTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// RelativeHumidity
func NewRelativeHumiditySubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback RelativeHumiditySubscriptionCallback) (*RelativeHumiditySubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg RelativeHumidity
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, RelativeHumidityTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CRelativeHumiditySequence = C.sensor_msgs__msg__RelativeHumidity__Sequence

func RelativeHumiditySequenceToGo(goSlice *[]RelativeHumidity, cSlice CRelativeHumiditySequence) {
	if cSlice.size == 0 {
		return
	}
...
func RelativeHumiditySequenceToC(cSlice *CRelativeHumiditySequence, goSlice []RelativeHumidity) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func RelativeHumidityArrayToGo(goSlice []RelativeHumidity, cSlice []CRelativeHumidity) {
	for i := 0; i < len(cSlice); i++ {
		RelativeHumidityTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func RelativeHumidityArrayToC(cSlice []CRelativeHumidity, goSlice []RelativeHumidity) {
