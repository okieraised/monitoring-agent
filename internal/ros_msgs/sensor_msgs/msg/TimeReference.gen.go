...
// TimeReference
func NewTimeReferencePublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*TimeReferencePublisher, error) {
	pub, err := node.NewPublisher(topicName, TimeReferenceTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// TimeReference
func NewTimeReferenceSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback TimeReferenceSubscriptionCallback) (*TimeReferenceSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg TimeReference
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, TimeReferenceTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CTimeReferenceSequence = C.sensor_msgs__msg__TimeReference__Sequence

func TimeReferenceSequenceToGo(goSlice *[]TimeReference, cSlice CTimeReferenceSequence) {
	if cSlice.size == 0 {
		return
	}
...
func TimeReferenceSequenceToC(cSlice *CTimeReferenceSequence, goSlice []TimeReference) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func TimeReferenceArrayToGo(goSlice []TimeReference, cSlice []CTimeReference) {
	for i := 0; i < len(cSlice); i++ {
		TimeReferenceTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func TimeReferenceArrayToC(cSlice []CTimeReference, goSlice []TimeReference) {
