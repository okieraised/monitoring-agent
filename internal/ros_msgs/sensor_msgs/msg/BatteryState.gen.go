...
// BatteryState
func NewBatteryStatePublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*BatteryStatePublisher, error) {
	pub, err := node.NewPublisher(topicName, BatteryStateTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// BatteryState
func NewBatteryStateSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback BatteryStateSubscriptionCallback) (*BatteryStateSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg BatteryState
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, BatteryStateTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
func (t _BatteryStateTypeSupport) AsCStruct(dst unsafe.Pointer, msg types.Message) {
	...
	primitives.Float32SequenceToC((*primitives.CFloat32Sequence)(unsafe.Pointer(&mem.cell_voltage)), m.CellVoltage)
	primitives.Float32SequenceToC((*primitives.CFloat32Sequence)(unsafe.Pointer(&mem.cell_temperature)), m.CellTemperature)
...
func (t _BatteryStateTypeSupport) AsGoStruct(msg types.Message, ros2_message_buffer unsafe.Pointer) {
	...
	primitives.Float32SequenceToGo(&m.CellVoltage, *(*primitives.CFloat32Sequence)(unsafe.Pointer(&mem.cell_voltage)))
	primitives.Float32SequenceToGo(&m.CellTemperature, *(*primitives.CFloat32Sequence)(unsafe.Pointer(&mem.cell_temperature)))
...
type CBatteryStateSequence = C.sensor_msgs__msg__BatteryState__Sequence

func BatteryStateSequenceToGo(goSlice *[]BatteryState, cSlice CBatteryStateSequence) {
	if cSlice.size == 0 {
		return
	}
...
func BatteryStateSequenceToC(cSlice *CBatteryStateSequence, goSlice []BatteryState) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func BatteryStateArrayToGo(goSlice []BatteryState, cSlice []CBatteryState) {
	for i := 0; i < len(cSlice); i++ {
		BatteryStateTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func BatteryStateArrayToC(cSlice []CBatteryState, goSlice []BatteryState) {
