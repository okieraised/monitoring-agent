...
// CameraInfo
func NewCameraInfoPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*CameraInfoPublisher, error) {
	pub, err := node.NewPublisher(topicName, CameraInfoTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// CameraInfo
func NewCameraInfoSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback CameraInfoSubscriptionCallback) (*CameraInfoSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg CameraInfo
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, CameraInfoTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
func (t _CameraInfoTypeSupport) AsCStruct(dst unsafe.Pointer, msg types.Message) {
	...
	primitives.Float64SequenceToC((*primitives.CFloat64Sequence)(unsafe.Pointer(&mem.d)), m.D)
	cSlice_k := mem.k[:]
	primitives.Float64ArrayToC(*(*[]primitives.CFloat64)(unsafe.Pointer(&cSlice_k)), m.K[:])
	cSlice_r := mem.r[:]
	primitives.Float64ArrayToC(*(*[]primitives.CFloat64)(unsafe.Pointer(&cSlice_r)), m.R[:])
	cSlice_p := mem.p[:]
	primitives.Float64ArrayToC(*(*[]primitives.CFloat64)(unsafe.Pointer(&cSlice_p)), m.P[:])
...
func (t _CameraInfoTypeSupport) AsGoStruct(msg types.Message, ros2_message_buffer unsafe.Pointer) {
	...
	primitives.Float64SequenceToGo(&m.D, *(*primitives.CFloat64Sequence)(unsafe.Pointer(&mem.d)))
	cSlice_k := mem.k[:]
	primitives.Float64ArrayToGo(m.K[:], *(*[]primitives.CFloat64)(unsafe.Pointer(&cSlice_k)))
	cSlice_r := mem.r[:]
	primitives.Float64ArrayToGo(m.R[:], *(*[]primitives.CFloat64)(unsafe.Pointer(&cSlice_r)))
	cSlice_p := mem.p[:]
	primitives.Float64ArrayToGo(m.P[:], *(*[]primitives.CFloat64)(unsafe.Pointer(&cSlice_p)))
...
type CCameraInfoSequence = C.sensor_msgs__msg__CameraInfo__Sequence

func CameraInfoSequenceToGo(goSlice *[]CameraInfo, cSlice CCameraInfoSequence) {
	if cSlice.size == 0 {
		return
	}
...
func CameraInfoSequenceToC(cSlice *CCameraInfoSequence, goSlice []CameraInfo) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func CameraInfoArrayToGo(goSlice []CameraInfo, cSlice []CCameraInfo) {
	for i := 0; i < len(cSlice); i++ {
		CameraInfoTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func CameraInfoArrayToC(cSlice []CCameraInfo, goSlice []CameraInfo) {
