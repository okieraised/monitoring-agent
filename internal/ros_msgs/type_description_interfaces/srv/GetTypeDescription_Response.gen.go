...
import (
	...
	type_description_interfaces_msg "github.com/okieraised/monitoring-agent/internal/ros_msgs/type_description_interfaces/msg"
...
type GetTypeDescription_Response struct {
	Successful       bool                                            `yaml:"successful"`        // True if the type description information is available and populated in the response.If false, all other fields except `failure_reason` are considered undefined.
	FailureReason    string                                          `yaml:"failure_reason"`    // If `successful` is false, contains a reason for failure.If `successful` is true, this is left empty.
	TypeDescription  type_description_interfaces_msg.TypeDescription `yaml:"type_description"`  // The parsed type description which can be used programmatically.
	TypeSources      []type_description_interfaces_msg.TypeSource    `yaml:"type_sources"`      // A list containing the interface definition source text of the requested type,plus all types it recursively depends on.Each source text is a copy of the original contents of the.msg, .srv, .action, .idl, or other file if it exists, including comments and whitespace.Sources can be matched with IndividualTypeDescriptions by their `type_name`.The `encoding` field of each entry informs how to interpret its contents.
	ExtraInformation []type_description_interfaces_msg.KeyValue      `yaml:"extra_information"` // Key-value pairs of extra information.
}
...
func NewGetTypeDescription_ResponsePublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*GetTypeDescription_ResponsePublisher, error) {
	pub, err := node.NewPublisher(topicName, GetTypeDescription_ResponseTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// GetTypeDescription_Response
func NewGetTypeDescription_ResponseSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback GetTypeDescription_ResponseSubscriptionCallback) (*GetTypeDescription_ResponseSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg GetTypeDescription_Response
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, GetTypeDescription_ResponseTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
func (t _GetTypeDescription_ResponseTypeSupport) AsCStruct(dst unsafe.Pointer, msg types.Message) {
	...
	type_description_interfaces_msg.TypeSourceSequenceToC((*type_description_interfaces_msg.CTypeSourceSequence)(unsafe.Pointer(&mem.type_sources)), m.TypeSources)
	type_description_interfaces_msg.KeyValueSequenceToC((*type_description_interfaces_msg.CKeyValueSequence)(unsafe.Pointer(&mem.extra_information)), m.ExtraInformation)
}
...
func (t _GetTypeDescription_ResponseTypeSupport) AsGoStruct(msg types.Message, ros2_message_buffer unsafe.Pointer) {
	...
	type_description_interfaces_msg.TypeSourceSequenceToGo(&m.TypeSources, *(*type_description_interfaces_msg.CTypeSourceSequence)(unsafe.Pointer(&mem.type_sources)))
	type_description_interfaces_msg.KeyValueSequenceToGo(&m.ExtraInformation, *(*type_description_interfaces_msg.CKeyValueSequence)(unsafe.Pointer(&mem.extra_information)))
}
...
type CGetTypeDescription_ResponseSequence = C.type_description_interfaces__srv__GetTypeDescription_Response__Sequence

func GetTypeDescription_ResponseSequenceToGo(goSlice *[]GetTypeDescription_Response, cSlice CGetTypeDescription_ResponseSequence) {
	if cSlice.size == 0 {
		return
	}
...
func GetTypeDescription_ResponseSequenceToC(cSlice *CGetTypeDescription_ResponseSequence, goSlice []GetTypeDescription_Response) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func GetTypeDescription_ResponseArrayToGo(goSlice []GetTypeDescription_Response, cSlice []CGetTypeDescription_Response) {
	for i := 0; i < len(cSlice); i++ {
		GetTypeDescription_ResponseTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func GetTypeDescription_ResponseArrayToC(cSlice []CGetTypeDescription_Response, goSlice []GetTypeDescription_Response) {
