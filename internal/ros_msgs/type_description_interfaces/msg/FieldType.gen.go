...
type FieldType struct {
	TypeId         uint8  `yaml:"type_id"`          // Identifying number for the type of the field, using one of the above constants.
	Capacity       uint64 `yaml:"capacity"`         // Only used when the type is an array or a bounded sequence.In the case of an array, this is the fixed capacity of the array.In the case of a bounded sequence, this is the maximum capacity of the sequence.In all other cases this field is unused.
	StringCapacity uint64 `yaml:"string_capacity"`  // Only used when the type is a fixed or bounded string/wstring, or a array/sequence of those.In the case of a fixed string/wstring, it is the fixed length of the string.In the case of a bounded string/wstring, it is the maximum capacity of the string.In the case of an array/sequence of fixed string/wstring, it is the fixed length of the strings.In the case of an array/sequence of bounded string/wstring, it is the maximum capacity of the strings.It is not currently possible to have different string capacities per element in the array/sequence.
	NestedTypeName string `yaml:"nested_type_name"` // Only used when the type is a nested type or array/sequence of nested types.This is limited to 255 characters.TODO(wjwwood): this 255 character limit was chosen due to this being the limitfor DDSI-RTPS based middlewares, which is the most commonly used right now.We lack a ROS 2 specific limit in our design documents, but we should updatethis and/or link to the design doc when that is available.
}
...
// FieldType
func NewFieldTypePublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*FieldTypePublisher, error) {
	pub, err := node.NewPublisher(topicName, FieldTypeTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// FieldType
func NewFieldTypeSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback FieldTypeSubscriptionCallback) (*FieldTypeSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg FieldType
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, FieldTypeTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CFieldTypeSequence = C.type_description_interfaces__msg__FieldType__Sequence

func FieldTypeSequenceToGo(goSlice *[]FieldType, cSlice CFieldTypeSequence) {
	if cSlice.size == 0 {
		return
	}
...
func FieldTypeSequenceToC(cSlice *CFieldTypeSequence, goSlice []FieldType) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func FieldTypeArrayToGo(goSlice []FieldType, cSlice []CFieldType) {
	for i := 0; i < len(cSlice); i++ {
		FieldTypeTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func FieldTypeArrayToC(cSlice []CFieldType, goSlice []FieldType) {
