...
// TypeSource
func NewTypeSourcePublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*TypeSourcePublisher, error) {
	pub, err := node.NewPublisher(topicName, TypeSourceTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// TypeSource
func NewTypeSourceSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback TypeSourceSubscriptionCallback) (*TypeSourceSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg TypeSource
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, TypeSourceTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CTypeSourceSequence = C.type_description_interfaces__msg__TypeSource__Sequence

func TypeSourceSequenceToGo(goSlice *[]TypeSource, cSlice CTypeSourceSequence) {
	if cSlice.size == 0 {
		return
	}
...
func TypeSourceSequenceToC(cSlice *CTypeSourceSequence, goSlice []TypeSource) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func TypeSourceArrayToGo(goSlice []TypeSource, cSlice []CTypeSource) {
	for i := 0; i < len(cSlice); i++ {
		TypeSourceTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func TypeSourceArrayToC(cSlice []CTypeSource, goSlice []TypeSource) {
