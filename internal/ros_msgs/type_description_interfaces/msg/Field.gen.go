...
type Field struct {
	Name         string    `yaml:"name"`          // Name of the field.
	Type         FieldType `yaml:"type"`          // Type of the field, including details about the type like length, nested name, etc.
	DefaultValue string    `yaml:"default_value"` // Literal default value of the field as a string, as it appeared in the originalmessage description file, whether that be .msg/.srv/.action or .idl.
}
...
// Field
func NewFieldPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*FieldPublisher, error) {
	pub, err := node.NewPublisher(topicName, FieldTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// Field
func NewFieldSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback FieldSubscriptionCallback) (*FieldSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg Field
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, FieldTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CFieldSequence = C.type_description_interfaces__msg__Field__Sequence

func FieldSequenceToGo(goSlice *[]Field, cSlice CFieldSequence) {
	if cSlice.size == 0 {
		return
	}
...
func FieldSequenceToC(cSlice *CFieldSequence, goSlice []Field) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func FieldArrayToGo(goSlice []Field, cSlice []CField) {
	for i := 0; i < len(cSlice); i++ {
		FieldTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func FieldArrayToC(cSlice []CField, goSlice []Field) {
