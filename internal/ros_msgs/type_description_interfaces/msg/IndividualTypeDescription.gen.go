...
type IndividualTypeDescription struct {
	TypeName string  `yaml:"type_name"` // Name of the type.This is limited to 255 characters.TODO(wjwwood): this 255 character limit was chosen due to this being the limitfor DDSI-RTPS based middlewares, which is the most commonly used right now.We lack a ROS 2 specific limit in our design documents, but we should updatethis and/or link to the design doc when that is available.
	Fields   []Field `yaml:"fields"`    // Fields of the type.
}
...
func NewIndividualTypeDescriptionPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*IndividualTypeDescriptionPublisher, error) {
	pub, err := node.NewPublisher(topicName, IndividualTypeDescriptionTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// IndividualTypeDescription
func NewIndividualTypeDescriptionSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback IndividualTypeDescriptionSubscriptionCallback) (*IndividualTypeDescriptionSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg IndividualTypeDescription
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, IndividualTypeDescriptionTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
func (t _IndividualTypeDescriptionTypeSupport) AsCStruct(dst unsafe.Pointer, msg types.Message) {
	...
	FieldSequenceToC(&mem.fields, m.Fields)
}
...
func (t _IndividualTypeDescriptionTypeSupport) AsGoStruct(msg types.Message, ros2_message_buffer unsafe.Pointer) {
	...
	FieldSequenceToGo(&m.Fields, mem.fields)
}
...
type CIndividualTypeDescriptionSequence = C.type_description_interfaces__msg__IndividualTypeDescription__Sequence

func IndividualTypeDescriptionSequenceToGo(goSlice *[]IndividualTypeDescription, cSlice CIndividualTypeDescriptionSequence) {
	if cSlice.size == 0 {
		return
	}
...
func IndividualTypeDescriptionSequenceToC(cSlice *CIndividualTypeDescriptionSequence, goSlice []IndividualTypeDescription) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func IndividualTypeDescriptionArrayToGo(goSlice []IndividualTypeDescription, cSlice []CIndividualTypeDescription) {
	for i := 0; i < len(cSlice); i++ {
		IndividualTypeDescriptionTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func IndividualTypeDescriptionArrayToC(cSlice []CIndividualTypeDescription, goSlice []IndividualTypeDescription) {
