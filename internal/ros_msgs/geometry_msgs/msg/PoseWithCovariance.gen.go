...
// PoseWithCovariance
func NewPoseWithCovariancePublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*PoseWithCovariancePublisher, error) {
	pub, err := node.NewPublisher(topicName, PoseWithCovarianceTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// PoseWithCovariance
func NewPoseWithCovarianceSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback PoseWithCovarianceSubscriptionCallback) (*PoseWithCovarianceSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg PoseWithCovariance
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, PoseWithCovarianceTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
func (t _PoseWithCovarianceTypeSupport) AsCStruct(dst unsafe.Pointer, msg types.Message) {
	...
	primitives.Float64ArrayToC(*(*[]primitives.CFloat64)(unsafe.Pointer(&cSlice_covariance)), m.Covariance[:])
}
...
func (t _PoseWithCovarianceTypeSupport) AsGoStruct(msg types.Message, ros2_message_buffer unsafe.Pointer) {
	...
	cSlice_covariance := mem.covariance[:]
	primitives.Float64ArrayToGo(m.Covariance[:], *(*[]primitives.CFloat64)(unsafe.Pointer(&cSlice_covariance)))
}
...
type CPoseWithCovarianceSequence = C.geometry_msgs__msg__PoseWithCovariance__Sequence

func PoseWithCovarianceSequenceToGo(goSlice *[]PoseWithCovariance, cSlice CPoseWithCovarianceSequence) {
	if cSlice.size == 0 {
		return
	}
...
func PoseWithCovarianceSequenceToC(cSlice *CPoseWithCovarianceSequence, goSlice []PoseWithCovariance) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func PoseWithCovarianceArrayToGo(goSlice []PoseWithCovariance, cSlice []CPoseWithCovariance) {
	for i := 0; i < len(cSlice); i++ {
		PoseWithCovarianceTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func PoseWithCovarianceArrayToC(cSlice []CPoseWithCovariance, goSlice []PoseWithCovariance) {
