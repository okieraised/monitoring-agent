...
// Pose2D
func NewPose2DPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*Pose2DPublisher, error) {
	pub, err := node.NewPublisher(topicName, Pose2DTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// Pose2D
func NewPose2DSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback Pose2DSubscriptionCallback) (*Pose2DSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg Pose2D
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, Pose2DTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CPose2DSequence = C.geometry_msgs__msg__Pose2D__Sequence

func Pose2DSequenceToGo(goSlice *[]Pose2D, cSlice CPose2DSequence) {
	if cSlice.size == 0 {
		return
	}
...
func Pose2DSequenceToC(cSlice *CPose2DSequence, goSlice []Pose2D) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func Pose2DArrayToGo(goSlice []Pose2D, cSlice []CPose2D) {
	for i := 0; i < len(cSlice); i++ {
		Pose2DTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func Pose2DArrayToC(cSlice []CPose2D, goSlice []Pose2D) {
