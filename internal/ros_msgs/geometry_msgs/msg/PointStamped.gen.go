...
// PointStamped
func NewPointStampedPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*PointStampedPublisher, error) {
	pub, err := node.NewPublisher(topicName, PointStampedTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// PointStamped
func NewPointStampedSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback PointStampedSubscriptionCallback) (*PointStampedSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg PointStamped
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, PointStampedTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CPointStampedSequence = C.geometry_msgs__msg__PointStamped__Sequence

func PointStampedSequenceToGo(goSlice *[]PointStamped, cSlice CPointStampedSequence) {
	if cSlice.size == 0 {
		return
	}
...
func PointStampedSequenceToC(cSlice *CPointStampedSequence, goSlice []PointStamped) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func PointStampedArrayToGo(goSlice []PointStamped, cSlice []CPointStamped) {
	for i := 0; i < len(cSlice); i++ {
		PointStampedTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func PointStampedArrayToC(cSlice []CPointStamped, goSlice []PointStamped) {
