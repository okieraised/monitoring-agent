...
// PolygonStamped
func NewPolygonStampedPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*PolygonStampedPublisher, error) {
	pub, err := node.NewPublisher(topicName, PolygonStampedTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// PolygonStamped
func NewPolygonStampedSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback PolygonStampedSubscriptionCallback) (*PolygonStampedSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg PolygonStamped
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, PolygonStampedTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CPolygonStampedSequence = C.geometry_msgs__msg__PolygonStamped__Sequence

func PolygonStampedSequenceToGo(goSlice *[]PolygonStamped, cSlice CPolygonStampedSequence) {
	if cSlice.size == 0 {
		return
	}
...
func PolygonStampedSequenceToC(cSlice *CPolygonStampedSequence, goSlice []PolygonStamped) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func PolygonStampedArrayToGo(goSlice []PolygonStamped, cSlice []CPolygonStamped) {
	for i := 0; i < len(cSlice); i++ {
		PolygonStampedTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func PolygonStampedArrayToC(cSlice []CPolygonStamped, goSlice []PolygonStamped) {
