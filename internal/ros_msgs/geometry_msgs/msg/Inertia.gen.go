...
// Inertia
func NewInertiaPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*InertiaPublisher, error) {
	pub, err := node.NewPublisher(topicName, InertiaTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// Inertia
func NewInertiaSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback InertiaSubscriptionCallback) (*InertiaSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg Inertia
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, InertiaTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CInertiaSequence = C.geometry_msgs__msg__Inertia__Sequence

func InertiaSequenceToGo(goSlice *[]Inertia, cSlice CInertiaSequence) {
	if cSlice.size == 0 {
		return
	}
...
func InertiaSequenceToC(cSlice *CInertiaSequence, goSlice []Inertia) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func InertiaArrayToGo(goSlice []Inertia, cSlice []CInertia) {
	for i := 0; i < len(cSlice); i++ {
		InertiaTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func InertiaArrayToC(cSlice []CInertia, goSlice []Inertia) {
