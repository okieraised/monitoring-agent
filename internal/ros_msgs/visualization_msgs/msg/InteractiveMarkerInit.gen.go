...
func NewInteractiveMarkerInitPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*InteractiveMarkerInitPublisher, error) {
	pub, err := node.NewPublisher(topicName, InteractiveMarkerInitTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// InteractiveMarkerInit
func NewInteractiveMarkerInitSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback InteractiveMarkerInitSubscriptionCallback) (*InteractiveMarkerInitSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg InteractiveMarkerInit
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, InteractiveMarkerInitTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CInteractiveMarkerInitSequence = C.visualization_msgs__msg__InteractiveMarkerInit__Sequence

func InteractiveMarkerInitSequenceToGo(goSlice *[]InteractiveMarkerInit, cSlice CInteractiveMarkerInitSequence) {
	if cSlice.size == 0 {
		return
	}
...
func InteractiveMarkerInitSequenceToC(cSlice *CInteractiveMarkerInitSequence, goSlice []InteractiveMarkerInit) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func InteractiveMarkerInitArrayToGo(goSlice []InteractiveMarkerInit, cSlice []CInteractiveMarkerInit) {
	for i := 0; i < len(cSlice); i++ {
		InteractiveMarkerInitTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func InteractiveMarkerInitArrayToC(cSlice []CInteractiveMarkerInit, goSlice []InteractiveMarkerInit) {
