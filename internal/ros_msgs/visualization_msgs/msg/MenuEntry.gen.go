...
type MenuEntry struct {
	...
	CommandType uint8 `yaml:"command_type"`
}
...
// MenuEntry
func NewMenuEntryPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*MenuEntryPublisher, error) {
	pub, err := node.NewPublisher(topicName, MenuEntryTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// MenuEntry
func NewMenuEntrySubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback MenuEntrySubscriptionCallback) (*MenuEntrySubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg MenuEntry
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, MenuEntryTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CMenuEntrySequence = C.visualization_msgs__msg__MenuEntry__Sequence

func MenuEntrySequenceToGo(goSlice *[]MenuEntry, cSlice CMenuEntrySequence) {
	if cSlice.size == 0 {
		return
	}
...
func MenuEntrySequenceToC(cSlice *CMenuEntrySequence, goSlice []MenuEntry) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func MenuEntryArrayToGo(goSlice []MenuEntry, cSlice []CMenuEntry) {
	for i := 0; i < len(cSlice); i++ {
		MenuEntryTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func MenuEntryArrayToC(cSlice []CMenuEntry, goSlice []MenuEntry) {
