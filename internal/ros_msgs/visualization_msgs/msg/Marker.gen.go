...
type Marker struct {
	...
	Points                   []geometry_msgs_msg.Point       `yaml:"points"`           // Only used if the type specified has some use for them (eg. POINTS, LINE_STRIP, ARROW_STRIP, etc.)
	...
	TextureResource          string                          `yaml:"texture_resource"` // Texture resource is a special URI that can either reference a texture file ina format acceptable to (resource retriever)[https://docs.ros.org/en/rolling/p/resource_retriever/]or an embedded texture via a string matching the format:"embedded://texture_name"
...
// Marker
func NewMarkerPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*MarkerPublisher, error) {
...
// Marker
func NewMarkerSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback MarkerSubscriptionCallback) (*MarkerSubscription, error) {
