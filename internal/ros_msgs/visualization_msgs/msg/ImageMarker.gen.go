...
// ImageMarker
func NewImageMarkerPublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*ImageMarkerPublisher, error) {
	pub, err := node.NewPublisher(topicName, ImageMarkerTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// ImageMarker
func NewImageMarkerSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback ImageMarkerSubscriptionCallback) (*ImageMarkerSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg ImageMarker
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, ImageMarkerTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
func (t _ImageMarkerTypeSupport) AsCStruct(dst unsafe.Pointer, msg types.Message) {
	...
	std_msgs_msg.ColorRGBASequenceToC((*std_msgs_msg.CColorRGBASequence)(unsafe.Pointer(&mem.outline_colors)), m.OutlineColors)
}
...
func (t _ImageMarkerTypeSupport) AsGoStruct(msg types.Message, ros2_message_buffer unsafe.Pointer) {
	...
	std_msgs_msg.ColorRGBASequenceToGo(&m.OutlineColors, *(*std_msgs_msg.CColorRGBASequence)(unsafe.Pointer(&mem.outline_colors)))
}
...
type CImageMarkerSequence = C.visualization_msgs__msg__ImageMarker__Sequence

func ImageMarkerSequenceToGo(goSlice *[]ImageMarker, cSlice CImageMarkerSequence) {
	if cSlice.size == 0 {
		return
	}
...
func ImageMarkerSequenceToC(cSlice *CImageMarkerSequence, goSlice []ImageMarker) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func ImageMarkerArrayToGo(goSlice []ImageMarker, cSlice []CImageMarker) {
	for i := 0; i < len(cSlice); i++ {
		ImageMarkerTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func ImageMarkerArrayToC(cSlice []CImageMarker, goSlice []ImageMarker) {
