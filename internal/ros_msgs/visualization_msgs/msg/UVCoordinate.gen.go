...
type UVCoordinate struct {
	U float32 `yaml:"u"` // Location of the pixel as a ratio of the width of a 2D texture.Values should be in range: [0.0-1.0].
	V float32 `yaml:"v"`
}
...
// UVCoordinate
func NewUVCoordinatePublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*UVCoordinatePublisher, error) {
	pub, err := node.NewPublisher(topicName, UVCoordinateTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// UVCoordinate
func NewUVCoordinateSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback UVCoordinateSubscriptionCallback) (*UVCoordinateSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg UVCoordinate
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, UVCoordinateTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CUVCoordinateSequence = C.visualization_msgs__msg__UVCoordinate__Sequence

func UVCoordinateSequenceToGo(goSlice *[]UVCoordinate, cSlice CUVCoordinateSequence) {
	if cSlice.size == 0 {
		return
	}
...
func UVCoordinateSequenceToC(cSlice *CUVCoordinateSequence, goSlice []UVCoordinate) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func UVCoordinateArrayToGo(goSlice []UVCoordinate, cSlice []CUVCoordinate) {
	for i := 0; i < len(cSlice); i++ {
		UVCoordinateTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func UVCoordinateArrayToC(cSlice []CUVCoordinate, goSlice []UVCoordinate) {
