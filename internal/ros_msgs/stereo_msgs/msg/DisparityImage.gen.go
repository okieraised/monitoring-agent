...
// DisparityImage
func NewDisparityImagePublisher(node *rclgo.Node, topicName string, options *rclgo.PublisherOptions) (*DisparityImagePublisher, error) {
	pub, err := node.NewPublisher(topicName, DisparityImageTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// DisparityImage
func NewDisparityImageSubscription(node *rclgo.Node, topicName string, opts *rclgo.SubscriptionOptions, subscriptionCallback DisparityImageSubscriptionCallback) (*DisparityImageSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg DisparityImage
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topicName, DisparityImageTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
...
type CDisparityImageSequence = C.stereo_msgs__msg__DisparityImage__Sequence

func DisparityImageSequenceToGo(goSlice *[]DisparityImage, cSlice CDisparityImageSequence) {
	if cSlice.size == 0 {
		return
	}
...
func DisparityImageSequenceToC(cSlice *CDisparityImageSequence, goSlice []DisparityImage) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
...
func DisparityImageArrayToGo(goSlice []DisparityImage, cSlice []CDisparityImage) {
	for i := 0; i < len(cSlice); i++ {
		DisparityImageTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}

func DisparityImageArrayToC(cSlice []CDisparityImage, goSlice []DisparityImage) {
